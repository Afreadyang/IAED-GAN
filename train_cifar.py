...
from models import Generator1, Discriminator, CifarCNN
...
def main(args):
    ...
    target_model = CifarCNN().cuda()
    target_model.load_state_dict(torch.load("./CNN/cifar_cnn.tar")["state_dict"])
    target_model.eval()

    check_path = args.checkpoint
    os.makedirs(check_path, exist_ok=True)
    fgsm = torch.load('./CIFAR10_AE/Training_Set/fgsm.tar')
    ...
    writer = SummaryWriter('./log/cifar')
    print_str = "\t".join(["{}"] + ["{:.6f}"] * 2)
    ...
    for e in range(epochs):
        ...
        for x, x_label, x_adv in tqdm(train_loader, total=len(train_loader), leave=False):
            ...
            # Train D
            y_real, y_real_label = D(x)
            x_fake = G(x_adv)
            y_fake, y_fake_label = D(x_fake)
            gradient_penalty = compute_gradient_penalty(D, x.data, x_fake.data)
            lambda_gp = 10
            cls = 0.5*(auxiliary_loss(y_real_label,x_real_label)+auxiliary_loss(y_fake_label,x_real_label))
            loss_D = -torch.mean(y_real.squeeze()) + torch.mean(y_fake.squeeze()) + lambda_gp * gradient_penalty + cls
            opt_D.zero_grad()
            loss_D.backward()
            opt_D.step()
            ...
            for _ in range(1):
                ...
                y_fake, y_fake_label= D(x_fake)
                # if e >100:
                loss_G = -torch.mean(y_fake.squeeze()) + 0.5*auxiliary_loss(target_model(x_fake),x_real_label) + 0.5*loss_mse(x_fake, x) + 0.5*auxiliary_loss(y_fake_label,x_real_label)
                # else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--checkpoint", type=str, default="./checkpoint/cifar")
